...
public final class QueryUtils {
    ...
    /* Commenting out the hardcoded JSON because now we would simply pass in the URL of USGS here
    ...
    */
    ...
    // The URL To be parsed is mentioned in the EarthquakeActivity.java file now.
    ...
    public static List<Earthquake> fetchEarthquakeData(String requestUrl) {
        ...
        // Try-catch added just for Testing the Loading Indicator
    ...
    /* The Entire method below was being used to navigate through the Hardcoded JSON and get the Data, but now that we are passing in the URL, the below code would be commented out
        ...
        // Catch the exception so the app doesn't crash, and print the error message to the logs.
            try
            {
                ...
                for(int i=0; i < jsonArray.length(); i++)
                {
                    ...
                    // Extract the value for the key called "mag", the magnitude is no longer being extracted as a string but its now a 'double' type
                    double magnitude = properties.getDouble("mag");
                    String loc = properties.getString("place");
                    long time = properties.getLong("time");

                    // Extract the value for the key called "url" from the JSON Response
                    String url = properties.getString("url");

                    // Create a new {@link Earthquake} object with the magnitude, location, time,
                    // and url from the JSON response.
                    Earthquake earthquake = new Earthquake(magnitude, loc, time, url);

                    // Earlier when we were not involving the intent of calling web browser on the clicking of a list item, the below code was valid
                    // Earthquake earthquake = new Earthquake(magnitude, loc, time);
